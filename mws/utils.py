# -*- coding: utf-8 -*-
"""
Utilities for python-amazon-mws
"""
...
class ObjectDict(dict):
    ...
    def __iter__(self):
        ...
        the node will create a list of ObjectDicts to that key.
        However, if we expect a list in downstream code when only a single item
        is returned, we will find a single ObjectDict. Attempting to iterate
        over that object will iterate through dict keys, which is not what we want.

        This override will send back an iterator of a list with a single element
        if necessary to allow iteration of any node with a single element.
        If accessing directly, we will still get a list or ObjectDict,
...
class XML2Dict(object):
    ...
    def _namespace_split(self, tag, value):
        """
        Split the namespace tag

        Example:
          for '{http://cs.sfsu.edu/csc867/myscheduler}patients':
          namespace = http://cs.sfsu.edu/csc867/myscheduler
          name = patients
        """
